// src/mqtt/sensors/CO2Handler.js
...
class CO2Handler extends BaseSensorHandler {
    async handleCO2Data(topic, messageValue) {
        ...
        try {
            const validation = this.validateNumeric(messageValue, 0, 10000);
            if (!validation.valid) return;

            const value = validation.value;
            await this.updateSensorCache('co2_level', value);
            ...
            for (const [userId, locations] of this.activeUsers) {
                for (const location of locations) {
                    try {
                        ...
                        console.log(`✅ Stored CO2 for user ${userId}`);
                    } catch (error) {
                    ...
                    console.log(`📡 CO2 emitted to user ${userId}`);
                }
            }

            console.log(`🫧 ========== END CO2 LEVEL ==========\n`);
        } catch (error) {
            console.error(`❌ Error handling CO2:`, error);
        }
    }
}
