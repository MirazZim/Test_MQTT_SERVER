// src/mqtt/sensors/SonarDistanceHandler.js
const BaseSensorHandler = require('../base/BaseSensorHandler');
const pool = require('../../config/db');

class SonarDistanceHandler extends BaseSensorHandler {
    async handleSonarDistance(topic, messageValue) {
        console.log(`\n📏 ========== SONAR DISTANCE ==========`);
        console.log(`📏 Value: ${messageValue}`);
        console.log(`📏 Active users: ${this.activeUsers.size}`);

        try {
            const validation = this.validateNumeric(messageValue, 0, 400);
            if (!validation.valid) return;

            const value = validation.value;
            await this.updateSensorCache('sonar_distance', value);
            ...
            for (const [userId, locations] of this.activeUsers) {
                for (const location of locations) {
                    try {
                        ...
                        console.log(`✅ Stored Sonar for user ${userId}`);
                    } catch (error) {
                    ...
                    console.log(`📡 Sonar emitted to user ${userId}`);
                }
            }

            console.log(`📏 ========== END SONAR DISTANCE ==========\n`);
        } catch (error) {
            console.error(`❌ Error handling sonar:`, error);
        }
    }
}
